...
import { Logo, Avatar, IconButton } from "@/components/ds";
...
export default function AppShell({ active, onNavigate, children }) {
  ...
  const name = profile?.name || "عضو منصة تلخيص";
  const role = profile?.role === "admin" ? "مدير النظام" : "عضو";
  const used = profile?.summaryCount ?? 0;
  const quota = 10;
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", background: "var(--surface-page)" }}>
      {/* Header */}
      <header style={{
        height: "var(--header-h)", flexShrink: 0, background: "var(--surface-card)",
        borderBottom: "1px solid var(--border-subtle)", display: "flex", alignItems: "center",
        gap: "20px", padding: "0 24px",
      }}>
        <Logo variant="full" size={22} />
        <div style={{
          flex: 1, maxWidth: "420px", marginInlineStart: "16px", display: "flex", alignItems: "center", gap: "8px",
          background: "var(--gray-50)", border: "1px solid var(--border-subtle)", borderRadius: "var(--radius-md)", padding: "8px 12px",
        }}>
          <Icon name="search" size={16} color="var(--text-subtle)" />
          <input placeholder="ابحث في المخاطبات والملخّصات…" style={{
            flex: 1, border: "none", background: "transparent", outline: "none", font: "inherit",
            fontSize: "var(--fs-sm)", color: "var(--text-body)",
          }} />
          <kbd style={{ fontFamily: "var(--font-mono)", fontSize: "11px", color: "var(--text-subtle)",
            background: "var(--surface-card)", border: "1px solid var(--border-subtle)", borderRadius: "4px", padding: "1px 6px" }}>⌘K</kbd>
        </div>
        <div style={{ flex: 1 }} />
        {profile?.role === "admin" && (
          <IconButton label="لوحة المسؤول" variant="ghost" onClick={() => router.push("/admin")}><Icon name="shield-check" /></IconButton>
        )}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "10px", paddingInlineStart: "10px",
          ...
          <div style={{ lineHeight: 1.2 }}>
            ...
            <div style={{ fontSize: "var(--fs-2xs)", color: "var(--text-muted)" }}>{role}</div>
          </div>
          <IconButton label="تسجيل الخروج" variant="ghost" onClick={handleLogout}><Icon name="log-out" /></IconButton>
        </div>
      </header>
      ...
      <div style={{ flex: 1, display: "flex", minHeight: 0 }}>
        ...
        <nav style={{
          ...
          {nav.map((n) => {
            ...
            return (
              <button key={n.id} onClick={() => onNavigate(n.id)} style={{
                ...
                border: "none", cursor: "pointer", textAlign: "right", borderRadius: "var(--radius-md)", font: "inherit",
                fontSize: "var(--fs-sm)", fontWeight: on ? "var(--fw-semibold)" : "var(--fw-medium)",
                background: on ? "var(--surface-brand-subtle)" : "transparent",
                color: on ? "var(--navy-800)" : "var(--text-muted)", transition: "background 120ms ease",
              }}
          ...
          <div style={{ flex: 1 }} />
          <div style={{ background: "var(--surface-inverse)", borderRadius: "var(--radius-lg)", padding: "16px", color: "var(--white)" }}>
            <div style={{ fontWeight: "var(--fw-semibold)", fontSize: "var(--fs-sm)", marginBottom: "4px" }}>الباقة المجانية</div>
            <div style={{ fontSize: "var(--fs-2xs)", color: "var(--navy-200)", marginBottom: "12px" }}>{used} من {quota} ملخّصات هذا الشهر</div>
